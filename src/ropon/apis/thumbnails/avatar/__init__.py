...
class RenderAvatar:
    ...
    @with_auth_token
    def render_headshot(self, user_id: int | None, thumbnail_size: Literal["48x48", "50x50", "60x60", "75x75", "100x100", "150x150", "180x180", "352x352", "420x420", "720x720"], **kwargs):
        try:
    ...
    @with_auth_token
    def render_headshot_bust(self, user_id: int | None, thumbnail_size: Literal["48x48", "50x50", "60x60", "75x75", "100x100", "150x150", "180x180", "352x352", "420x420"], **kwargs):
        try:
            ...
            headers = {".ROBLOSECURITY": self.auth_token} if self.auth_token else {}
            req = requests.get(
                f"https://thumbnails.roblox.com/v1/users/avatar-bust?userIds={user_id}&size={thumbnail_size}&format=Png&isCircular=false",
                headers=headers
            )
            req.raise_for_status()
            response = req.json()
    ...
    @with_auth_token
    def render_outfit(self, outfit_id: int | None, thumbnail_size: Literal["150x150", "450x450"], **kwargs):
        try:
            ...
            headers = {".ROBLOSECURITY": self.auth_token} if self.auth_token else {}
            req = requests.get(
                f"https://thumbnails.roblox.com/v1/users/outfits?useroutfitIds={outfit_id}&size={thumbnail_size}&format=Png&isCircular=false",
                headers=headers
            )
            req.raise_for_status()
            response = req.json()
